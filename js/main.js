...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // 1. Configurar entorno de Monaco para soporte offline
...
// ===== CONFIGURACIÓN MONACO OFFLINE =====
function setupMonacoEnvironment() {
  window.MonacoEnvironment = {
    getWorkerUrl: function(moduleId, label) {
      return `data:text/javascript;charset=utf-8,${encodeURIComponent(`
        self.MonacoEnvironment = {
        baseUrl: '/monaco-cache/'
        };
        try {
        importScripts('/monaco-cache/vs/base/worker/workerMain.js');
        } catch (error) {
        console.error('Failed to load worker from cache, using CDN fallback');
...
async function loadMonacoEditor() {
  // Verificar si tenemos recursos de Monaco en cache
  const useCached = await caches.has(MONACO_CACHE);
